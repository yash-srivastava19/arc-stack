...
6. **Handle exit code 3 (conflict) explicitly.** After a conflict during `arc sync`, `arc rebase`, `arc drop`, or `arc land`, use `arc rebase --continue`/`--abort`. `--continue` resumes the whole cascade (not just the one branch that conflicted); `--abort` rolls every branch in the run back to its pre-cascade SHA, not just the one in progress. For `arc drop`/`arc land`, `--continue` only finishes the *rebase* — re-run `arc drop <branch> --force` / `arc land <branch> --force` afterward to complete the operation (safe: already-rebased branches are no-ops on the re-run). `arc rebase --continue`/`--abort` also work as a generic fallback for a bare (non-cascade) rebase left paused by `arc restack`. After a conflict during `arc edit`, use `arc edit --continue`/`--abort`.
